...
def arm2_ik(x_, l_, th_):
    X = np.matrix(((np.array(x_[0])),(np.array(x_[1]))));
    # 収束計算を50回繰り返す
    for j in range(20):
        # 原点座標(縦ベクトル)
        vec = np.array([[0.],[0.],[1.]] );
        ...
        # 現在の手先位置を求める
        Xg = P*Rz(th_[0])*L(l_[0])*Rz(th_[1])*L(l_[1])*vec;
        # ヤコビ行列を求める
        J1 = dRz(th_[0])*L(l_[0])*vec;
        J2 = Rz(th_[0])*L(l_[0])*dRz(th_[1])*L(l_[1])*vec;
...
def arm2_fk(l_, th_):
    vec = np.array([[0.],[0.],[1.]] );
    (x1, y1, z1) = Rz(th_[0])*L(l_[0])*vec;                # 第1関節の位置
    (x2, y2, z2) = Rz(th_[0])*L(l_[0])*Rz(th_[1])*L(l_[1])*vec;  # 第2関節の位置
    return x1, y1, x2, y2
